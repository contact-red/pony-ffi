...
class CBuffer
  ...
  var _written: CBox[ISize] = CBox[ISize](0)
  ...
  fun get_written_size(): ISize =>
    """
    Most recent filled length, in bytes. Signed to allow caller-defined sentinels.
    """
    _written.value

  fun ref set_written_size(n: ISize) =>
    """
  ...
  fun ref written_size_ptr(): CBox[ISize] =>
    """
  ...
  fun ref copy_string_truncated(): String iso^ ? =>
    ...
    """
    if _written.value < 0 then error end
